...
def dqn_learning(dem_array, rirsv_array, wkmstrm_array, road_array, watershed_basins_array, channels_array, reward_calculator, action_mode='8_directions', load_existing=False, model_filename='dqn_model.pth'):
    global epsilon
    ...
    for episode in range(episodes):
        ...
        while not done and step < max_steps:
            ...
            if action_mode == '8_directions':
                if action == 0:  # 상
                    next_x, next_y = (x - speed, y)
                elif action == 1:  # 하
                    next_x, next_y = (x + speed, y)
                elif action == 2:  # 좌
                    next_x, next_y = (x, y - speed)
                elif action == 3:  # 우
                    next_x, next_y = (x, y + speed)
                elif action == 4:  # 좌상
...
def simulate_path(start_x, start_y, model, dem_array, rirsv_array, wkmstrm_array, road_array, watershed_basins_array, channels_array, action_mode='8_directions'):
